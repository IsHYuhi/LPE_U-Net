...
class Loader(object):
    ...
    def generate_paths(dir_original, dir_segmented):
        ...
        paths_original = list(map(lambda filename: dir_original + "/" + filename + ".png", filenames))#ファイルネーム.jpgにする
    ...
    def extract_images(paths_original, paths_segmented, init_size, one_hot):
        ...
        for image, _ in Loader.image_generator(paths_segmented, init_size, normalization=False):
            ...
            #今回のデータセットはラベルの割り当てが0, 5, 10なので0, 1, 2に変更。
            image = np.where(image == 5, 1, image)#labelの変更
            image = np.where(image == 10, 2, image)#labelの変更
